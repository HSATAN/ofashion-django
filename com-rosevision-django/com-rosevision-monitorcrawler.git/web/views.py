...
def chart(request, brand_id=None):
    ...
    brands = CrawlInfo.objects.exclude(brand_id__in=[10006, 10080, 10105, 10108, 10114, 10117, 10138, 10186,
                                                     10218, 10204, 10305, 10376, 10617, 10767, 10880, 11401,
...
def table(request):
    ...
    for item in brand_info:
        ...
        brand_info_dict[k0] = item.brandname_e

    all_crawl_infos = {}
    for each in temp:
        if each.region in ['cn','us','fr','uk','hk','it','de','ch','au']:
            ...
            if (not region in all_crawl_infos[brand_id][brand_name]):
                all_crawl_infos[brand_id][brand_name][region] = [{'flag':0, 'original_no':'-', 'touch_no':'',  'and_chart':''} for i in xrange(10)]

            cur_unit = all_crawl_infos[brand_id][brand_name][region][time_index]
            cur_unit['original_no'] = original_no
            cur_unit['touch_no'] = touch_no
            cur_unit['and_chart'] = '/'
            if (float(original_no) - 0.0 > 0.5 and float(touch_no)/float(original_no) < 0.15):
                cur_unit['flag'] = 1
            else:
                cur_unit['flag'] = 0
    for brand in brands:
